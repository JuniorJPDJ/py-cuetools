...
def mb_discid(toc: List):
    toc_iter = mb_toc(toc)
    first = next(toc_iter)
    last = next(toc_iter)
    out = f'{first:02X}{last:02X}' + ''.join(f'{t:08X}' for t in toc_iter)

    return base64.b64encode(hashlib.sha1(out.ljust(804, '0').encode()).digest()).decode('ascii').replace('+', '.').replace('/', '_').replace('=', '-')
...
def crc32_wonull(b):
    ...
    for i in range(0, len(b), 2):
        if b[i] != 0 and b[i+1] != 0:
            crc = zlib.crc32(b[i:i+2], crc)
    return crc

def crc_fun():
    for i in range(1, 17):
        with open(f'{i:02}.pcm', 'rb') as f:
            pcm = f.read()
            data = pcm
            print(f"Track {i}:")
            print(f"CRC32: {zlib.crc32(data):08x}")
            print(f"CRC32 W/O NULL: {crc32_wonull(pcm):08x}")
...
def main(fname):
    ...
    print("MusicBrainZ DiscID:", mb_discid(toc))
